...
void draw(TString name, double cell_size, double min, double max){
	...
	for(int i=0; i<40; i++){
		...
		mytree->Project("tmp","x_Cali_Cell");
		x[i] = tmp->GetMean();			
	...
	TCanvas *c1 = new TCanvas("c1","relation between x_Cali and x_True",200,10,500,400);
...
//	std::cout << "par0: " << f->GetParameter(0) << "  par1: " << f->GetParameter(1) << std::endl;
	...
	gr->SetTitle("after S correction "+name);
	gr->GetXaxis()->SetTitle("x cali cluster position/[mm]");
...
//	f->Draw("same");
#if 0
	auto legend = new TLegend(0.12,0.65,0.50,0.85);
...
void correction(){
	...
	gStyle->SetOptStat(0);
	gStyle->SetOptFit(0); 	
	draw("inner",40.4,-8.5,8.5);
	draw("middle",60.6,-18,18);
